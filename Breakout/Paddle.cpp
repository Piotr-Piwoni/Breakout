...
Paddle::Paddle(sf::RenderWindow* window) : _window(window),
										   _width(PADDLE_WIDTH), _isAlive(true),
										   _timeInNewSize(0.0f)
{
	_sprite.setFillColor(sf::Color::Cyan);
	_sprite.setPosition((window->getSize().x - _width) / 2.0f,
...
void Paddle::moveRight(float dt)
{
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::D) && position < _window->
		getSize().x - _width)
	{
		_sprite.move(sf::Vector2f(dt * PADDLE_SPEED, 0));
	}
