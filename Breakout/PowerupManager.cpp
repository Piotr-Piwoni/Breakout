...
PowerupManager::PowerupManager(sf::RenderWindow* window, Paddle* paddle,
							   Ball* ball) : _window(window), _paddle(paddle),
											 _ball(ball)
{}
...
void PowerupManager::update(float dt)
{
	...
	if (_powerupInEffect)
	{
		...
		if (_powerupInEffect->second.y <= 0)
		{
...
{
	return static_cast<int>(_powerups.size());
}

std::pair<POWERUPS, sf::Vector2f> PowerupManager::getPowerupInEffect()
{
	if (!_powerupInEffect)
		return {none, {0.0f, 0.0f}};
