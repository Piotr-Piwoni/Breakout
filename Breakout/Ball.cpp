...
Ball::Ball(sf::RenderWindow* window, float velocity, GameManager* gameManager) :
	_direction({1, 1}), _window(window), _velocity(velocity),
	_isAlive(true), _isFireBall(false), _timeWithPowerupEffect(0.f),
...
void Ball::update(float dt)
{
	...
	if (_isFireBall)
	{
		...
		_sprite.setFillColor(sf::Color(flicker, flicker / 2, 0));
		// Orange flickering color
	}
	...
	// bounce on walls
	if ((position.x >= windowDimensions.x - 2 * RADIUS && _direction.x > 0) || (
			position.x <= 0 && _direction.x < 0))
	{
	...
	if (_sprite.getGlobalBounds().intersects(
			_gameManager->getPaddle()->getBounds()))
	{
		SoundManager::Get().Play("bop");
		_direction.y *= -1; // Bounce vertically

		float paddlePositionProportion =
				(_sprite.getPosition().x - _gameManager->getPaddle()->
				 getBounds().left) /
				_gameManager->getPaddle()->getBounds().width;
		_direction.x = paddlePositionProportion * 2.0f - 1.0f;

		// Adjust position to avoid getting stuck inside the paddle
		_sprite.setPosition(_sprite.getPosition().x,
							_gameManager->getPaddle()->getBounds().top - 2 *
							RADIUS);
	}

	// collision with bricks
	int collisionResponse = _gameManager->getBrickManager()->checkCollision(
			_sprite, _direction);
	if (_isFireBall)
